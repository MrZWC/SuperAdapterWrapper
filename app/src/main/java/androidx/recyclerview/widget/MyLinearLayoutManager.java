...
public class MyLinearLayoutManager extends RecyclerView.LayoutManager implements
     ...
     * the same object.
     */
    private MyLinearLayoutManager.LayoutState mLayoutState;
    ...
    MyLinearLayoutManager.SavedState mPendingSavedState = null;
     ...
     */
    final MyLinearLayoutManager.AnchorInfo mAnchorInfo = new MyLinearLayoutManager.AnchorInfo();

    /**
     * Stashed to avoid allocation, currently only used in #fill()
     */
    private final MyLinearLayoutManager.LayoutChunkResult mLayoutChunkResult = new MyLinearLayoutManager.LayoutChunkResult();
    ...
    public Parcelable onSaveInstanceState() {
        if (mPendingSavedState != null) {
            return new MyLinearLayoutManager.SavedState(mPendingSavedState);
        }
        MyLinearLayoutManager.SavedState state = new MyLinearLayoutManager.SavedState();
        if (getChildCount() > 0) {
            ensureLayoutState();
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof MyLinearLayoutManager.SavedState) {
            mPendingSavedState = (MyLinearLayoutManager.SavedState) state;
            requestLayout();
            if (DEBUG) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mAnchorInfo.mLayoutFromEnd) {
            firstLayoutDirection = mShouldReverseLayout ? MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_TAIL
                    : MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_HEAD;
        } else {
            firstLayoutDirection = mShouldReverseLayout ? MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_HEAD
                    : MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;
        }
    ...
    void onAnchorReady(RecyclerView.Recycler recycler, RecyclerView.State state,
                       MyLinearLayoutManager.AnchorInfo anchorInfo, int firstLayoutItemDirection) {
    }
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < scrapSize; i++) {
            ...
            final int direction = position < firstChildPos != mShouldReverseLayout
                    ? MyLinearLayoutManager.LayoutState.LAYOUT_START : MyLinearLayoutManager.LayoutState.LAYOUT_END;
            if (direction == MyLinearLayoutManager.LayoutState.LAYOUT_START) {
    ...
    private void updateAnchorInfoForLayout(RecyclerView.Recycler recycler, RecyclerView.State state,
                                           MyLinearLayoutManager.AnchorInfo anchorInfo) {
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler,
                                             RecyclerView.State state, MyLinearLayoutManager.AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
        final View focused = getFocusedChild();
     ...
     */
    private boolean updateAnchorFromPendingData(RecyclerView.State state, MyLinearLayoutManager.AnchorInfo anchorInfo) {
        if (state.isPreLayout() || mPendingScrollPosition == RecyclerView.NO_POSITION) {
            return false;
        }
    ...
    private void updateLayoutStateToFillEnd(MyLinearLayoutManager.AnchorInfo anchorInfo) {
        updateLayoutStateToFillEnd(anchorInfo.mPosition, anchorInfo.mCoordinate);
    }

    private void updateLayoutStateToFillEnd(int itemPosition, int offset) {
        mLayoutState.mAvailable = mOrientationHelper.getEndAfterPadding() - offset;
        mLayoutState.mItemDirection = mShouldReverseLayout ? MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_HEAD :
                MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_TAIL;
        mLayoutState.mCurrentPosition = itemPosition;
        mLayoutState.mLayoutDirection = MyLinearLayoutManager.LayoutState.LAYOUT_END;
        mLayoutState.mOffset = offset;
        mLayoutState.mScrollingOffset = MyLinearLayoutManager.LayoutState.SCROLLING_OFFSET_NaN;
    }

    private void updateLayoutStateToFillStart(MyLinearLayoutManager.AnchorInfo anchorInfo) {
    ...
    private void updateLayoutStateToFillStart(int itemPosition, int offset) {
        ...
        mLayoutState.mCurrentPosition = itemPosition;
        mLayoutState.mItemDirection = mShouldReverseLayout ? MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_TAIL :
                MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_HEAD;
        mLayoutState.mLayoutDirection = MyLinearLayoutManager.LayoutState.LAYOUT_START;
        mLayoutState.mOffset = offset;
        mLayoutState.mScrollingOffset = MyLinearLayoutManager.LayoutState.SCROLLING_OFFSET_NaN;
     ...
     */
    MyLinearLayoutManager.LayoutState createLayoutState() {
        return new MyLinearLayoutManager.LayoutState();
    }
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
        ...
        int scrollingOffset;
        if (layoutDirection == MyLinearLayoutManager.LayoutState.LAYOUT_END) {
    ...
    void collectPrefetchPositionsForLayoutState(RecyclerView.State state, MyLinearLayoutManager.LayoutState layoutState,
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state,
        ...
        ensureLayoutState();
        final int layoutDirection = delta > 0 ? MyLinearLayoutManager.LayoutState.LAYOUT_END : MyLinearLayoutManager.LayoutState.LAYOUT_START;
    ...
    int scrollBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        ensureLayoutState();
        final int layoutDirection = dy > 0 ? MyLinearLayoutManager.LayoutState.LAYOUT_END : MyLinearLayoutManager.LayoutState.LAYOUT_START;
     ...
     * @see MyLinearLayoutManager.LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(RecyclerView.Recycler recycler, MyLinearLayoutManager.LayoutState layoutState) {
        if (!layoutState.mRecycle || layoutState.mInfinite) {
            return;
        }
        if (layoutState.mLayoutDirection == MyLinearLayoutManager.LayoutState.LAYOUT_START) {
     ...
     */
    int fill(RecyclerView.Recycler recycler, MyLinearLayoutManager.LayoutState layoutState,
             RecyclerView.State state, boolean stopOnFocusable) {
        // max offset we should set is mFastScroll + available
        final int start = layoutState.mAvailable;
        if (layoutState.mScrollingOffset != MyLinearLayoutManager.LayoutState.SCROLLING_OFFSET_NaN) {
        ...
        MyLinearLayoutManager.LayoutChunkResult layoutChunkResult = mLayoutChunkResult;
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
                     MyLinearLayoutManager.LayoutState layoutState, MyLinearLayoutManager.LayoutChunkResult result) {
        ...
        if (layoutState.mScrapList == null) {
            if (mShouldReverseLayout == (layoutState.mLayoutDirection
                    == MyLinearLayoutManager.LayoutState.LAYOUT_START)) {
                addView(view);
            } else {
                addView(view, 0);
            }
     ...
     * @return {@link MyLinearLayoutManager.LayoutState#LAYOUT_START} or {@link MyLinearLayoutManager.LayoutState#LAYOUT_END} if focus direction
     * is applicable to current state, {@link MyLinearLayoutManager.LayoutState#INVALID_LAYOUT} otherwise.
     */
    int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            case View.FOCUS_BACKWARD:
                if (mOrientation == VERTICAL) {
                    return MyLinearLayoutManager.LayoutState.LAYOUT_START;
                } else if (isLayoutRTL()) {
                    return MyLinearLayoutManager.LayoutState.LAYOUT_END;
                } else {
                    return MyLinearLayoutManager.LayoutState.LAYOUT_START;
                }
            ...
            case View.FOCUS_UP:
                return mOrientation == VERTICAL ? MyLinearLayoutManager.LayoutState.LAYOUT_START
                        : MyLinearLayoutManager.LayoutState.INVALID_LAYOUT;
            case View.FOCUS_DOWN:
                return mOrientation == VERTICAL ? MyLinearLayoutManager.LayoutState.LAYOUT_END
                        : MyLinearLayoutManager.LayoutState.INVALID_LAYOUT;
            case View.FOCUS_LEFT:
                return mOrientation == HORIZONTAL ? MyLinearLayoutManager.LayoutState.LAYOUT_START
                        : MyLinearLayoutManager.LayoutState.INVALID_LAYOUT;
            case View.FOCUS_RIGHT:
                return mOrientation == HORIZONTAL ? MyLinearLayoutManager.LayoutState.LAYOUT_END
                        : MyLinearLayoutManager.LayoutState.INVALID_LAYOUT;
            default:
                if (DEBUG) {
                    Log.d(TAG, "Unknown focus request:" + focusDirection);
                }
                return MyLinearLayoutManager.LayoutState.INVALID_LAYOUT;
        }
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
        ...
        if (layoutDir == MyLinearLayoutManager.LayoutState.INVALID_LAYOUT) {
            return null;
        }
        ensureLayoutState();
        ensureLayoutState();
        ...
        if (layoutDir == MyLinearLayoutManager.LayoutState.LAYOUT_START) {
    ...
    public void prepareForDrop(@NonNull View view, @NonNull View target, int x, int y) {
        ...
        final int dropDirection = myPos < targetPos ? MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_TAIL
                : MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_HEAD;
        if (mShouldReverseLayout) {
            if (dropDirection == MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_TAIL) {
            ...
            if (dropDirection == MyLinearLayoutManager.LayoutState.ITEM_DIRECTION_HEAD) {
    ...
    public static class SavedState implements Parcelable {
        ...
        public SavedState(MyLinearLayoutManager.SavedState other) {
        ...
        public static final Parcelable.Creator<MyLinearLayoutManager.SavedState> CREATOR =
                new Parcelable.Creator<MyLinearLayoutManager.SavedState>() {
                    @Override
                    public MyLinearLayoutManager.SavedState createFromParcel(Parcel in) {
                        return new MyLinearLayoutManager.SavedState(in);
                    }

                    @Override
                    public MyLinearLayoutManager.SavedState[] newArray(int size) {
                        return new MyLinearLayoutManager.SavedState[size];
                    }
                };
    }
